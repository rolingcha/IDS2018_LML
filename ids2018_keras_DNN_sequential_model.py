# -*- coding: utf-8 -*-
"""
Created on Mon Sep 14 09:26:48 2020
...
import ids2018_data_set_wrangling as ids2018_wrangling
...
def parsing_argv(argv):
    using_max_normal_value = False
    using_dropout_rate = False
...
def load_data_set(data_set_tag="0K"):
    ...
    """
    return ids2018_wrangling.get_X_raw_y_raw(data_set_tag)
...
def make_up_X(X_raw=None, kind_of_columns=""):
    """Gathering the columns to be used as X inputs based on what
        kind of columns will be considered
    ...
    rows_with_value_exceed_float64 : numpy array
    
    """
    # Scalars Features
    scalars_df = ids2018_wrangling.get_wrangled_column(X_raw, column_name='scalars')
    row_col_value_exceed_float64 = np.where(scalars_df.values >= np.finfo(np.float64).max)
    rows_with_value_exceed_float64 = np.unique(row_col_value_exceed_float64[0])
    scalars_df = scalars_df.drop(index=rows_with_value_exceed_float64)
    
    # Destination Port
    dst_port_df = ids2018_wrangling.get_wrangled_column(X_raw, column_name='Dst Port')
    dst_port_df = dst_port_df.drop(index=rows_with_value_exceed_float64)
    
    # Protocol 
    protocol_df = ids2018_wrangling.get_wrangled_column(X_raw, column_name='Protocol')
    protocol_df = protocol_df.drop(index=rows_with_value_exceed_float64)
    
    selected_data_set_npa = []

    # selecting features to be use as input (scalars, categorical, or both)
    if 'categorical' == kind_of_columns:
        selected_data_set_df = pd.concat([dst_port_df, protocol_df], axis=1, sort=False)
        ...
        selected_data_set_npa = np.concatenate([scaled_scalars_npa,
                                               dst_port_df.to_numpy(),
                                               protocol_df.to_numpy()],
                                               axis=1)
        
    return selected_data_set_npa, rows_with_value_exceed_float64
...
def main():
    ...
    X, rows_with_value_exceed_float64 = make_up_X(X_raw, exec_params['kind-of-columns'])
    y_raw = y_raw.drop(index=rows_with_value_exceed_float64)
    y = make_up_y(y_raw)    
